...
test("server-renders the Attention teaching page", async () => {
  ...
  assert.match(html, /为什么相加，而不是拼接/);
  assert.match(html, /特定线性变换的一个简单特例/);
  assert.match(html, /单位矩阵，所以/);
  assert.match(html, /二维例子 · Token 3/);
  assert.match(html, /原始 Transformer 的实际计算/);
