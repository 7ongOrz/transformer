...
test("server-renders the Attention teaching page", async () => {
  ...
  assert.match(html, /one-hot 只负责指出位置/);
  assert.match(html, /位置分支 · 地址查表/);
  assert.match(html, /拼接后做一次分块线性变换/);
  assert.match(html, /one-hot 只是把位置编号转换成位置向量/);
  assert.match(html, /发生混合.*不等于.*位置信息消失/);
  assert.match(html, /原始 Transformer 的前向计算/);
  ...
  assert.doesNotMatch(html, /PDF 为什么|数学补充 · 不是|仅用于推导|临时写法|上一节|下一节|这一节只为|页面刷新|被引用几万次|原理看懂了|与上文/);
