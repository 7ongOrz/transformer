...
test("server-renders the Attention teaching page", async () => {
  ...
  assert.match(html, /为什么使用相加？从“拼接”推到“相加”/);
  assert.match(html, /拼接扩展输入/);
  assert.match(html, /分块线性层/);
  assert.match(html, /乘法拆成两项/);
  assert.match(html, /内容贡献/);
  assert.match(html, /位置贡献/);
  assert.match(html, /数字 1 只负责选中位置矩阵的一行/);
  assert.match(html, /位置由整行向量表示，不由某一个坐标表示/);
  assert.match(html, /one-hot 选择位置向量/);
  assert.match(html, /位置 3 的 one-hot 地址/);
  assert.match(html, /为什么宽度不变/);
