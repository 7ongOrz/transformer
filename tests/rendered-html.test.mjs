...
test("server-renders the Attention teaching page", async () => {
  ...
  assert.match(html, /等价线性表示/);
  assert.match(html, /相加可以写成拼接向量经过特定分块线性变换/);
  assert.match(html, /位置索引 · one-hot/);
  assert.match(html, /分块矩阵乘法的逐项展开/);
  assert.match(html, /该等价关系依赖特定分块结构/);
  assert.match(html, /一般拼接与任意线性映射不等价于直接相加/);
