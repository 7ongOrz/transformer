...
test("server-renders the Attention teaching page", async () => {
  ...
  assert.match(html, /先看一般线性变换，再看单位矩阵特例/);
  assert.match(html, /位置索引 · one-hot/);
  assert.match(html, /沿特征维拼接/);
  assert.match(html, /一般形式 · 此时不使用单位矩阵/);
  assert.match(html, /一般结论是/);
  assert.match(html, /教学数值特例 · 取/);
  assert.match(html, /单位矩阵只是让这个一般式化为/);
  assert.match(html, /等价解释与实际计算/);
