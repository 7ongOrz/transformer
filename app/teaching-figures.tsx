...
import { attentionDemo, multiHeadDemo } from "./attention-demo.js";

const tokenLabels = ["token₁", "token₂", "token₃", "token₄"];
const { V: values } = attentionDemo;
...
export const AttentionSetupGuide = memo(function AttentionSetupGuide() {
  return (
    <div className="setup-guide">
      ...
      <div className="setup-types">
        ...
        <div><b>数值算例</b><span>矩阵使用固定数值，便于逐项核算；这些数值不对应某个已训练模型。显示的小数经过舍入，计算保留更高精度，因此显示权重之和可能略偏离 1。</span></div>
      </div>
    </div>
  );
});

export const ScoreMatrixReadingGuide = memo(function ScoreMatrixReadingGuide() {
  return (
...
export const FigStageQKV = memo(function FigStageQKV() {
 ...
 * ============================================================ */
export const FigStageScore = memo(function FigStageScore() {
  ...
  return (
    <div className="fig">
      <div className="score-explainer" role="group" aria-label="用 Token 1 的 Query 与四个 Key 分别计算缩放点积，并组成分数矩阵 S 的第一行">
 ...
 * 数据直接来自统一数值链路。
 * ============================================================ */
export const FigStageSoftmax = memo(function FigStageSoftmax() {
  ...
  return (
    <div className="fig">
      <div className="softmax-figure" role="group" aria-label="整行 softmax：四个分数联合归一化为四个注意力权重">
        ...
        <div className="softmax-flow">
          ...
          <div className="softmax-arrow"><span>整行</span><b>→</b></div>
        ...
        <div className="stable-softmax">
          <Formula block tex={String.raw`A_{1,j}=\frac{e^{S_{1,j}}}{\sum_t e^{S_{1,t}}}=\frac{e^{S_{1,j}-m}}{\sum_t e^{S_{1,t}-m}},\quad m=\max_t S_{1,t}`} />
          <p>分子、分母同乘 <Formula tex={String.raw`e^{-m}`} />，比例不变，且指数输入不大于 0。FlashAttention 分块计算时也使用这一性质：最大值更新后，先把旧分子、分母换算到新尺度，再累加新块。</p>
        </div>
      </div>
...
export const FigStageAggregate = memo(function FigStageAggregate() {
  const weights = attentionDemo.A[0];
...
export const FigMatrixStage = memo(function FigMatrixStage() {
  return (
...
export const FigMultiHeadCalculation = memo(function FigMultiHeadCalculation() {
...
export const FigTransformer = memo(function FigTransformer() {
  const Arrow = TfArrow;
  const Box = TfBox;
  return (
    <div className="fig">
      <svg className="transformer-figure" viewBox="0 0 940 630" width="940" role="img" aria-label="经典 Transformer Encoder-Decoder 结构图">
        ...
        <Box x={575} y={174} w={240} h={50} fill="rgba(245,176,66,0.14)" stroke="#f5b042" label="Masked Multi-Head Attention" sub="允许自身与过去，屏蔽未来" lc="#f5b042" sc="#6e7aab" />
...
export const PositionEncodingFlow = memo(function PositionEncodingFlow() {
  ...
  return (
    <div className="position-demo">
      ...
      <div className="position-matrix-flow" role="group" aria-label="四个 Token 的内容向量逐行加上位置向量，得到 Attention 输入矩阵 X">
      ...
      <div className="position-addition-proof">
        ...
        <div className="position-proof-calculation" role="group" aria-label="取内容分块为二维单位矩阵后，Token 3 的拼接行向量经过分块线性变换得到内容与位置之和">
...
export const FigFlashCompare = memo(function FigFlashCompare() {
  return (
...
export const FigFlashGpuMap = memo(function FigFlashGpuMap() {
  return (
...
export const FigFlashNumericTiles = memo(function FigFlashNumericTiles() {
  return (
    <div className="fig flash-exact-demo">
      ...
      <div className="flash-exact-result">
        ...
        <div className="flash-equivalence-chain">
          ...
          <div className="flash-equivalence-step">
            <i>归一化后得到相同权重</i>
            <Formula block tex={String.raw`A_1^{\mathrm{Flash}}=\frac1{\ell_2}[\tfrac14,\ \tfrac12,\ \tfrac34,\ 1]=[0.1,\ 0.2,\ 0.3,\ 0.4]=A_1^{\text{普通}}`} />
          </div>
          <div className="flash-equivalence-step final">
            <i>最终输出相同</i>
            <Formula block tex={String.raw`O_1^{\mathrm{Flash}}=o_2/\ell_2=75/(\tfrac52)=30=O_1^{\text{普通}}`} />
          </div>
        </div>
      </div>
